...
//static constexpr char const * lifecycle_node = "lifetalker";
...
// static constexpr char const * node_get_state_topic = "lifetalker/get_state";
// static constexpr char const * node_change_state_topic = "lifetalker/change_state";
...
callee_script(std::shared_ptr<LifecycleServiceClient> lc_client)
{
  ...
  // activate it again
  {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Activating Again" );
    time_between_state_changes.sleep();
    if (!rclcpp::ok()) {
      return;
    }
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Deactivating Again" );
    time_between_state_changes.sleep();
    if (!rclcpp::ok()) {
      return;
    }
